-- Content Engine Analytics Tables

-- Main content analytics table
CREATE TABLE IF NOT EXISTS content_analytics (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    content_id TEXT NOT NULL, -- References content from localStorage or generated content
    content_type TEXT NOT NULL CHECK (content_type IN ('post', 'article', 'thread', 'other')),
    platform TEXT, -- 'LinkedIn', 'Twitter', 'Medium', etc.
    title TEXT,
    content_preview TEXT, -- First 200 chars for reference
    metadata JSONB DEFAULT '{}'::jsonb, -- Additional metadata (tone, expertise area, etc.)
...
CREATE TABLE IF NOT EXISTS content_performance (
    ...
    engagement_rate DECIMAL(5, 2) DEFAULT 0, -- Calculated as (likes + comments + shares) / views * 100
...
CREATE TABLE IF NOT EXISTS content_generation_stats (
    ...
    expertise_areas_used TEXT[], -- Array of expertise area names
    platforms_targeted TEXT[], -- Array of platform names
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(user_id, date)
);
