...
DROP POLICY IF EXISTS "Users can view their own brand audits" ON brand_audits;
CREATE POLICY "Users can view their own brand audits"
    ON brand_audits FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own brand audits" ON brand_audits;
CREATE POLICY "Users can insert their own brand audits"
    ON brand_audits FOR INSERT
    WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own brand audits" ON brand_audits;
CREATE POLICY "Users can update their own brand audits"
    ON brand_audits FOR UPDATE
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own brand audits" ON brand_audits;
...
DROP POLICY IF EXISTS "Users can view their own recommendation statuses" ON brand_recommendations_status;
...
DROP POLICY IF EXISTS "Users can insert their own recommendation statuses" ON brand_recommendations_status;
...
DROP POLICY IF EXISTS "Users can update their own recommendation statuses" ON brand_recommendations_status;
...
DROP TRIGGER IF EXISTS update_brand_audits_updated_at ON brand_audits;
CREATE TRIGGER update_brand_audits_updated_at
    BEFORE UPDATE ON brand_audits
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_brand_recommendations_status_updated_at ON brand_recommendations_status;
